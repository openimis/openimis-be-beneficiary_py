...
class ProcessUpdateBeneficiariesWorkflowTest(TestCase):
    ...
    @patch('beneficiary.apps.SocialProtectionConfig.enable_maker_checker_for_beneficiary_update', False)
    @skipIf(
        connection.vendor != "postgresql",
        "Skipping tests due to implementation usage of validate_json_schema, which is a postgres specific extension."
    )
    ...
    @patch('beneficiary.apps.SocialProtectionConfig.enable_maker_checker_for_beneficiary_update', True)
