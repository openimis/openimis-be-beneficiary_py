...
from beneficiary.models import IndividualDataSourceUpload
import json
...
class GroupBeneficiaryImportGQLTest(openIMISGraphQLTestCase, TaskDataMixin):
    ...
    def test_import_beneficiaries(self):
        ...
        response = self.client.post(
            "/api/beneficiary/import_beneficiaries/",
            data,
            format='multipart',
            headers=headers,
        )
        ...
        self.assertEqual(content['data']['mutationLogs']['edges'][0]['node']['status'], 2,
                         "Fail during Task Group assingnment")
        ...
        # TODO: Add assertion for beneficiary