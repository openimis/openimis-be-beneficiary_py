...
from beneficiary.workflows.utils import (
    SqlProcedurePythonWorkflow,
...
class TestBasePythonWorkflowExecutor(TestCase):
    ...
    def setUp(self):
        ...
        self.mock_load_dataframe = patch('beneficiary.workflows.utils.load_dataframe').start()
    ...
    def test_validate_dataframe_headers_invalid_headers(self):
        ...
        self.executor.df = pd.DataFrame(
            columns=['first_name', 'last_name', 'dob', 'id', 'location_name', 'location_code', 'unexpected_column'])
