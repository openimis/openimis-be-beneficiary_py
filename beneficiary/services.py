...
from beneficiary.utils import (
    load_dataframe,
    fetch_summary_of_valid_items,
    fetch_summary_of_broken_items
)
from beneficiary.validation import (
...
class BeneficiaryService(BaseService, CheckerLogicServiceMixin):
    ...
    def would_exceed_max_active_beneficiaries(self, benefit_plan_id, status, id=None):
        ...
        if benefit_plan and status == "ACTIVE":
            ...
            active_beneficiaries = Beneficiary.objects.filter(is_deleted=False, benefit_plan_id=benefit_plan_id,
    ...
    @register_service_signal('beneficiary.create')
    def create(self, obj_data):
        try:
            ...
            if self.would_exceed_max_active_beneficiaries(benefit_plan_id, status):
                raise ValueError(
                    f"Error creating beneficiary with active status. Benefit plan is already at max active beneficiaries")
            return super().create(obj_data)
        except Exception as exc:
            return output_exception(model_name=self.OBJECT_TYPE.__name__, method="update", exception=exc)

    @register_service_signal('beneficiary.update')
    def update(self, obj_data):
        try:
            ...
            if self.would_exceed_max_active_beneficiaries(benefit_plan_id, status, id):
                raise ValueError(
                    f"Error changing beneficiary to active status. Benefit plan is already at max active beneficiaries")
            return super().update(obj_data)
        except Exception as exc:
            return output_exception(model_name=self.OBJECT_TYPE.__name__, method="update", exception=exc)

    @register_service_signal('beneficiary.delete')
...
class GroupBeneficiaryService(BaseService, CheckerLogicServiceMixin):
    ...
    def would_exceed_max_active_beneficiaries(self, benefit_plan_id, status, id=None):
        ...
        if benefit_plan and status == "ACTIVE":
            max_active_beneficiaries = benefit_plan.max_beneficiaries
            active_beneficiaries = GroupBeneficiary.objects.filter(is_deleted=False, benefit_plan_id=benefit_plan_id,
                                                                   status="ACTIVE").distinct()
            beneficiary_already_active = active_beneficiaries.filter(id=id).exists() if id else False
            return active_beneficiaries.count() == max_active_beneficiaries and not beneficiary_already_active
        return False

    @register_service_signal('beneficiary_group.create')
    def create(self, obj_data):
        try:
            status = obj_data.get("status", None)
            benefit_plan_id = obj_data.get("benefit_plan_id", None)
    ...
    @register_service_signal('beneficiary_group.update')
    def update(self, obj_data):
        try:
            status = obj_data.get("status", None)
            benefit_plan_id = obj_data.get("benefit_plan_id", None)
    ...
    @register_service_signal('beneficiary_group.delete')
    def delete(self, obj_data):
        return super().delete(obj_data)
...
class BeneficiaryImportService:
    ...
    @register_service_signal('beneficiary_import.import_beneficiaries')
    ...
    def create_task_with_importing_valid_items(self, upload_id: uuid, benefit_plan: BenefitPlan):
        if BeneficiaryConfig.enable_maker_checker_for_beneficiary_upload:
            ...
            from beneficiary.signals.on_validation_import_valid_items import \
    ...
    def create_task_with_update_valid_items(self, upload_id: uuid, benefit_plan: BenefitPlan):
        if BeneficiaryConfig.enable_maker_checker_for_beneficiary_update:
            BeneficiaryTaskCreatorService(self.user) \
                .create_task_with_update_valid_items(upload_id, benefit_plan)

        # Resolve automatically if maker-checker not enabled
        if not BeneficiaryConfig.enable_maker_checker_for_beneficiary_update:
            from beneficiary.signals.on_validation_import_valid_items import \
    ...
    def _validate_possible_beneficiaries(self, dataframe: DataFrame, benefit_plan: BenefitPlan, upload_id: uuid):
        ...
        calculation_uuid = BeneficiaryConfig.validation_calculation_uuid
    ...
    def _handle_uniqueness(self, row, field, field_properties, benefit_plan, dataframe):
        unique_class_validation = BeneficiaryConfig.unique_class_validation
        calculation_uuid = BeneficiaryConfig.validation_calculation_uuid
        calculation = get_calculation_object(calculation_uuid)
...
class BeneficiaryTaskCreatorService:
    ...
    def create_task_with_importing_valid_items(self, upload_id: uuid, benefit_plan: BenefitPlan):
        self._create_task(benefit_plan, upload_id, BeneficiaryConfig.validation_import_valid_items)

    def create_task_with_update_valid_items(self, upload_id: uuid, benefit_plan: BenefitPlan):
        self._create_task(benefit_plan, upload_id, BeneficiaryConfig.validation_upload_valid_items)

    @register_service_signal('beneficiary_task.update_task')
    @transaction.atomic()
