...
@api_view(["GET"])
@permission_classes([check_user_rights(BeneficiaryConfig.gql_beneficiary_create_perms, )])
def download_template_benefit_plan_file(request):
    ...
    try:
        base_fields = BeneficiaryConfig.beneficiary_base_fields
...
def download_beneficiary_upload(request):
    try:
        ...
        target_file_path = BeneficiaryConfig.get_beneficiary_upload_file_path(benefit_plan_id, filename)
...
def _handle_file_upload(file, benefit_plan):
    try:
        target_file_path = BeneficiaryConfig.get_beneficiary_upload_file_path(benefit_plan.id, file.name)
        file_handler = DefaultStorageFileHandler(target_file_path)
