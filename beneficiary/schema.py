...
from beneficiary.gql_mutations import (
...
from beneficiary.gql_queries import (
    BeneficiaryGQLType,
...
class Query(ExportableBeneficiaryQueryMixin, graphene.ObjectType):
    export_patches = {
        'beneficiary': [
            patch_details
        ],
    ...
    def resolve_beneficiary(self, info, **kwargs):
        def _build_filters(info, **kwargs):
            ...
            Query._check_permissions(
                info.context.user,
                BeneficiaryConfig.gql_beneficiary_search_perms
            )
            return filters
